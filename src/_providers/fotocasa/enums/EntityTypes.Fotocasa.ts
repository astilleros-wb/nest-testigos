...
export const FotoToMetaTypology = (
  ...
  if (raw_type[0] === 2) {
    if (raw_type[1].every((s) => s in [3, 5, 9])) return [Typology.casa];
...
export const MetaToFotoTypologys = (values: number[], encode: 'api') => {
  const col = 3;
  ...
  if (
    values.indexOf(Typology.piso) >= 0 &&
    values.indexOf(Typology.casa) >= 0
  ) {
