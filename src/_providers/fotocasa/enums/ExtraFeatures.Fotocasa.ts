...
export const codeToExtraFeaturetr = Object.entries(ExtraFeaturetrToCode).reduce((p: {[key: number] : string}, v: [string, number]) =>{
    p[Number(v[1])] = v[0]
    return p
}, {})
...
export const codeToExtraFeatureNum = Object.entries(ExtraFeatureNumToCode).reduce((p: {[key: number] : number}, v: any[]) =>{
    p[Number(v[1])] = v[0]
    return p
}, {})
...
export const mapExtraFeatureFromScrap = (F: {
    key: string;
    value: [number];
    minValue: number;
    maxValue: number;
